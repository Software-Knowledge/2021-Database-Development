...
3. 关系代数也是一样
   1. 2、3、4 这些数字对应的就是 这些数字对应的就是 关系（表）
   2. +-×/ 这些运算符对应的就是 关系操作
4. 如果将所有的路径全部遍历完会有比较大的代价
5. 要对知识、应用环境的现在、未来、时间空间分布有了解

![](img/lec4/1.png)

## 1.2. 关系代数使据库变成了科学而不是艺术 关系代数使据库变成了科学而不是艺术
![](img/lec4/2.png)
...
## 1.5. 软解析
1. 目前主流的还是采用硬解析的方式，避免出现SQL注入的问题

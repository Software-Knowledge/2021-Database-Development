...
1. 这个时候，Transaction1能提交吗？

![](img/lec2/1.png)

## 3.3. Oracle存在有时读不到正确数据的现象
1. Oracle的多版本控制，读一致性的并发模型
   1. 读一致查询：对于一个时间点（point time），查询会产生一致的结果
   2. 非阻塞查询：查询不会被写入器阻塞，**但在其它数据库中可能不是这样的**

![](img/lec2/2.png)

## 3.4. Oracle这种锁机制的好处是什么？
1. 例子：ACCOUNTS（account_number, account_balance）一个银行的账户余额,为了简单，只考虑一个仅有四行的表（同时假设每个数据库块中只存放一行数据。）
...
## 3.5. Oracle和其它数据库在并发上的差别
![](img/lec2/3.png)

## 3.6. 从体系结构和特性中了解具体数据库的锁机制
1. 比如Oracle实现的锁机制
   1. 只有修改才加行级锁
   2. Read绝对不会对数据加锁
   3. Writer不会阻塞Reader
...
3. 请将你的思考和分析，完成课后的小作业（不是两次大作业），优秀的作业会予以表扬或其它奖励

# 4. 程序员的工作

## 4.1. 开发成功数据库应用的要点 -黑盒的问题
> 对大多数码农而言，据库锁机制好像都是自动和透明实现的那么深 入

## 4.2. 问题是：这对我们码农有什么影响吗？
...
5. 什么时候，码农需要自己考虑并发的问题？不知道，看情况

# 5. 黑盒的问题
1. 数据库**黑盒**和**独立性**的问题
2. 数据库的某些细微差异对**开发**造成影响
...
3. 要构建一个完全数据库**独立**的应用
   ...
   2. 如果你清楚每个数据库工作的具体细节，你就会知道，**数据库独立性**可能并不是你真正想要的

## 5.2. 例如：Null值造成的数据库迁移障碍
...
## 5.4. 思考题
1. 关于把数据库当成黑盒使用的错误，其实也会在你学习软件开发中遇到类似
的问题，比如，对操作系统的黑盒化，比如对某些开发框架的黑盒化等等,
请你思考一下，你的学习过程中，还能找到类似的例子嘛?
...
3. 问题三: IO和CPU负载都高
   1. 硬件不够用了
   2. SQL存在问题
   3. 性能问题，90%的问题来源都是程序员的问题，开发环境到生产环境是一场灾难

## 6.6. SQL优化的方向
![](img/lec2/6.png)

## 6.7. 思考
1. 你对你常用的关系数据库关系系统中，去寻找一些针对优化的工具，去尝试使用一些性能的分析和监控I具(查看数据库官方Reference，首先使用官方的命令和工具)
